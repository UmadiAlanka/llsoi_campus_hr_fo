  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/auth/login`, {
        ...
        // Try sending both username and email fields to the backend
        body: JSON.stringify({
          username,
          email: username, // Fallback in case the backend expects 'email'
          password
        }),
      });
      ...
      if (res.ok && data.success) {
        localStorage.setItem("user", JSON.stringify({
          // Prioritize numeric IDs for technical operations like attendance
          userId: data.emp_id || data.id || data.employeeId || data.userId || data.username,
